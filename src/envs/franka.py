...
from env.environment import BaseEnvironment, BaseEnvironmentConfig
from utils.franka import clamp_translation
...
class FrankaEnvironmentConfig(BaseEnvironmentConfig):
    ...
    physical_cameras: dict[str, tuple[str, str]] = field(
        default_factory=lambda: {
    ...
    env: Environment = Environment.PANDA
...
class FrankaEnv(BaseEnvironment):
    def __init__(self, config: FrankaEnvironmentConfig):
        ...
        assert config.image_size == config.realsense.resolution
    ...
    def _step(
        ...
        self.robot_pose.pose.position.x = clamp_translation(
            self.robot_pose.pose.position.x, delta_position[0], self.position_limits[0]
        )

        self.robot_pose.pose.position.y = clamp_translation(
            self.robot_pose.pose.position.y, delta_position[1], self.position_limits[1]
        )

        self.robot_pose.pose.position.z = clamp_translation(
            self.robot_pose.pose.position.z, delta_position[2], self.position_limits[2]
        )
    ...
    def get_obs(self):
        ...
        if self.eval:
            img_frames = [
                int_to_float_range(np_channel_back2front(f)) for f in img_frames
            ]
        else:
