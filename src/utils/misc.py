...
def policy_checkpoint_name(
    ...
    if create_suffix and config.policy_config.suffix:
        raise ValueError("Should not pass suffix AND ask to create one.")
    if v := config.policy_config.suffix:
        suffix = "-" + v
    elif create_suffix:
    ...
    policy_type = str(config.policy)

    if hasattr(config.policy_config, "encoder_name"):
        encoder_name = "_" + str(config.policy_config.encoder_name)
    else:
        encoder_name = ""

    if hasattr(config.policy_config, "encoder_naming"):
        pretrained_on = "_pon-" + config.policy_config.encoder_naming.task
    else:
        pretrained_on = ""
...
def correct_action(keyboard_obs, action):
    if keyboard_obs.is_direction_commanded:
